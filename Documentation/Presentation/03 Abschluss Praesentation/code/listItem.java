// Fill authors from database to list item
	...
	if (debugMode) {
		Log.d("TABAC", "DATABASE: " + anz.toString() + 
		" authors in table.");
	}
	if (anz > 0) {
		authorCrs.moveToFirst();
		do {
			int index = authorCrs.getColumnIndex(
			DatabaseInfo.AUTHORS_AUTHOR_COL);
			if (debugMode) {
				Log.d("TABAC", "Index (AUTHORS_AUTHOR): " 
				+ index);
			}
